...
class Rotor:
    ...
    def __init__(self, propeller: propeller.Propeller, relative_position_body_frame: np.ndarray, is_ccw_blade: bool) -> None:
        ...
        self.local_wind_velocity = np.zeros(3)         # local background wind speed at rotor center in FLU inertial frame (an interface for inflow model)
        self.sensed_wind_velocity = np.zeros(3)         # local gross wind speed at rotor center in FLU inertial frame (an interface for inflow model)
        self.f_rotor_inertial_frame = np.zeros(3)          # force generated by air dynamics in inertial frame, for debugging purpose, be written by disturbance model
        self.thrust = 0.0            # positive thrust lift the drone
